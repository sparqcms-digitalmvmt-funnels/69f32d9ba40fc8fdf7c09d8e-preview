...
function getNextPageSlugForRedirect() {
  const normalize = (value) => {
    if (!value) return "";
    return value.startsWith("/69f32d9ba40fc8fdf7c09d8e-preview") ? value : (value.startsWith("/") ? "/69f32d9ba40fc8fdf7c09d8e-preview" + value : "/69f32d9ba40fc8fdf7c09d8e-preview/" + value);
  };
...
async function createOrderViaWallet(confirmationToken, paymentMethodId) {
  ...
  const orderData = {
    pageId: "L4UdJVSJydPZinENzbXf_h0G1ROIHKtn_lMwMVRceV7BFpJ84ZDStV5PpT3aSDXk",
    action: "process",
...
const getStates = async (countryIso2Code) => {
  ...
  try {
    if (!countriesStatesCache) {
      const res = await fetch('/69f32d9ba40fc8fdf7c09d8e-preview/scripts/countriesData/countriesStates.json', { cache: 'no-store' });
